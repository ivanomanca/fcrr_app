...
public class SlideShowActivityArtifact extends ActivityArtifact {
	...
	@GUARD boolean isDownloaded(int i) {
		return mData[i]!=null;
	}
	
	
	@INTERNAL_OPERATION void downloadImage(int index) {
		// long term operation
...
//			await(new ImageDownloader(i));
//			if (i==0) { // set first image
//				mUrl = "http://dl.dropbox.com/u/11954389/img"+mIndex+".JPG";
//				Bitmap bmp = BitmapFactory.decodeByteArray(mData[mIndex], 0, mData[mIndex].length);
//				mActivity.setImage(bmp);
//			} 
//		}
	...
	@INTERNAL_OPERATION void onFling(MotionEvent me1, MotionEvent me2, float velocityX, float velocityY) {
	...
	@INTERNAL_OPERATION void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (requestCode == 0) {
	        if (resultCode == Activity.RESULT_OK) {
	            ...
	            //signal("device_fouded", "10.0.2.2:20102", "lipermi");
	...
	@INTERNAL_OPERATION void onOptionsItemSelected(MenuItem item) {
		signal("search_device");
	}
	
	@OPERATION void showMessage(String msg) {
		mActivity.showMessage(msg);
	}
	
	@OPERATION void getCurrentSlideData(OpFeedbackParam<byte[]> data) {
		data.set(mData[mIndex]);
	}
	
	@OPERATION void getCurrentSlideUrl(OpFeedbackParam<String> url) {
		url.set(mUrl);
	}
}
