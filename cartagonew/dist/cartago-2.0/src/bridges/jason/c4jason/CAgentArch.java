...
public class CAgentArch extends AgArch implements cartago.ICartagoListener {
	...
	public void act(ActionExec actionExec, List<ActionExec> feedback) {
		...
		//CartagoAction act = CartagoEnvironment.getInstance().getCartagoAction(actName);
		//act.execute(this.getAgName(), this, envSession, action, actionExec);
				...
				if (evt instanceof ActionSucceededEvent){
					...
					if (props!=null){
						for (ArtifactObsProperty prop: props){
							...
							try {
								//Literal lold = TermConversionUtils.toPercept(ev.getArtifactId(), propName, null, lib);
								...
								while (!found && it.hasNext()){
									...
									if (annots.size() > 0){
										for (Term annot: annots){
											...
											/*
											Object artId = TermConversionUtils.termToObject(((Structure)annot).getTerm(0),lib);
