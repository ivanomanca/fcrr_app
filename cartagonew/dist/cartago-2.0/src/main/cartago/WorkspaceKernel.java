...
public class WorkspaceKernel  {
	...
	private HashMap<String,CartagoContext> agentContextsMap;
	...
	private CartagoContext wspManager;
	 ...
	 * 
	 */
	WorkspaceKernel(WorkspaceId id){		
		this.id=id;
		isShutdown = false;
		eventRegistry = new EventRegistry();

		agentContextsMap = new HashMap<String,CartagoContext>();	
		...
		ctxIds = 0;
		wspManager = new CartagoContext(new AgentId("workspace-manager", ctxIds++, "WorkspaceManager",id),this,null);
	...
	public ICartagoContext joinWorkspace(AgentCredential cred, ICartagoCallback eventListener) throws CartagoException {
		...
		synchronized (agentContextsMap){
			for (java.util.Map.Entry<String,CartagoContext> e: agentContextsMap.entrySet()){
				if (e.getKey().equals(cred.getId())){
					throw new SecurityException("Duplicate identity.");
				}
			}
			ctxIds++;
			AgentId userId = new AgentId(cred.getId(), ctxIds, roleName,id);				
			CartagoContext context = new CartagoContext(userId, this, eventListener); 
			agentContextsMap.put(userId.getAgentName(),context);
			return context;
		}
	}

	public void quitUser(AgentId userId) throws CartagoException {
		synchronized (agentContextsMap){
			CartagoContext body = agentContextsMap.remove(userId.getAgentName());
			if (body==null){
	...
	public ArtifactId makeArtifact(AgentId userId, String name, String template, ArtifactConfig config) throws ArtifactAlreadyPresentException, UnknownArtifactTemplateException, ArtifactConfigurationFailedException {
		...
		try {
			...
			try {
				...
				String[] ops = desc.getAdapter().getUsageInterface();
				synchronized (opMap){
					for (String op: ops){
						List<ArtifactDescriptor> list = opMap.get(op);
						if (list == null){
				...
				if (logManager.isLogging()){
					logManager.logArtifactCreated(System.currentTimeMillis(),id);
				}	
	...
	public void disposeArtifact(AgentId uid, ArtifactId id) throws CartagoException {
		synchronized (artifactMap){
			ArtifactDescriptor des = artifactMap.remove(id.getName());
			if (des==null){
			...
			if (logManager.isLogging()){
				logManager.logArtifactDisposed(System.currentTimeMillis(),id);
			}			
		}
	}
	...
	public void linkArtifacts(AgentId userId, ArtifactId artifactOutId, String artifactOutPort, ArtifactId artifactInId) throws CartagoException  {
		...
		if (logManager.isLogging()){
			logManager.logActionLinkArtifactsExecuted(System.currentTimeMillis(),userId,artifactOutId,artifactInId);
		}
	}
	...
	HashMap<String,CartagoContext> getCurrentAgentContexts(){
		return this.agentContextsMap;
	}

	public void removeGarbageBody(CartagoContext ctx){
		try {
			quitUser(ctx.getAgentId());
		} catch (Exception ex){
	...
	private String[] getCurrentArtifacts(){
		synchronized(artifactMap){
			Set<String> set = artifactMap.keySet();
	...
	private String[] getCurrentAgents(){
		synchronized(agentContextsMap){
			Set<String> set = agentContextsMap.keySet();
			String[] names = new String[set.size()];
			set.toArray(names);
			return names;
		}
	...
	private boolean removeArtifact(String artifactName){
		synchronized (artifactMap){
			...
			if (des!=null) {
				...
				if (logManager.isLogging()){
					logManager.logArtifactDisposed(System.currentTimeMillis(),des.getArtifact().getId());
				}		
				return true;
			} else {
				return false;
	...
	private boolean removeUser(String userName){
		synchronized (agentContextsMap){
			CartagoContext body = agentContextsMap.remove(userName);
			return body!=null;
		}
	}

	private OperationInfo[] getOpInExecution(String artifactName) throws CartagoException {
		synchronized (artifactMap){
			ArtifactDescriptor des = artifactMap.get(artifactName);
			if (des!=null) {
				return des.getAdapter().getOpInExecution();
			} else {
	...
	private List<ArtifactObsProperty> getObsProperties(String artifactName) throws CartagoException {
		synchronized (artifactMap){
			ArtifactDescriptor des = artifactMap.get(artifactName);
			if (des!=null) {
				return des.getAdapter().readProperties();
			} else {
				throw new CartagoException();
			}
		}
	...
	private String[] getUsageInterface(String artifactName) throws CartagoException {
		synchronized (artifactMap){
			ArtifactDescriptor des = artifactMap.get(artifactName);
			if (des!=null) {
				return des.getAdapter().getUsageInterface();
			} else {
				throw new CartagoException();
			}
		}
	...
	class CartagoController implements ICartagoController {
		...
		public boolean removeUser(String userName) throws CartagoException {
			return env.removeUser(userName);
		}

		public OperationInfo[] getOpInExecution(String artifactName) throws CartagoException {
			return env.getOpInExecution(artifactName);
		}

		public List<ArtifactObsProperty> getObsProperties(String artifactName) throws CartagoException {
			return env.getObsProperties(artifactName);
		}

		public String[] getUsageInterface(String artifactName) throws CartagoException {
			return env.getUsageInterface(artifactName);
		}		
	}
