...
public class WorkspaceScene {
	...
	HashMap<String,FocussedArtifactInfo> focussedArt;
	HashMap<String,OpInfo> focusedArts;
	Random rand; 
    Lock lock;
	...
	public WorkspaceScene(){
		...
		focussedArt = new HashMap<String,FocussedArtifactInfo>();
	...
	public void newFocus(ArtifactId id, AgentId aid){
	    try {
	    	lock.lock();
	    	focussedArt.put(id.getName(),new FocussedArtifactInfo(aid,id));
	    } finally {
	    	lock.unlock();
	    }
	}
