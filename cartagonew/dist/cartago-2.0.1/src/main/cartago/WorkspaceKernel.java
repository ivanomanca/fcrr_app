...
public class WorkspaceKernel  {
	...
	WorkspaceKernel(WorkspaceId id, ICartagoLogger logger){		
		...
		ctxIds = 0;
		wspManager = new AgentBody(new AgentId("workspace-manager", ctxIds++, "WorkspaceManager",id),this,null);
	...
	public ICartagoContext joinWorkspace(AgentCredential cred, ICartagoCallback eventListener) throws CartagoException {
		...
		synchronized (agentContextsMap){
			for (java.util.Map.Entry<String,AgentBody> e: agentContextsMap.entrySet()){
			...
			AgentBody context = new AgentBody(userId, this, eventListener); 
	...
	public void quitAgent(AgentId userId) throws CartagoException {
		synchronized (agentContextsMap){
			AgentBody body = agentContextsMap.remove(userId.getAgentName());
			if (body==null){
	...
	private boolean removeUser(String userName){
		synchronized (agentContextsMap){
			AgentBody body = agentContextsMap.remove(userName);
			return body!=null;
		}
	}
