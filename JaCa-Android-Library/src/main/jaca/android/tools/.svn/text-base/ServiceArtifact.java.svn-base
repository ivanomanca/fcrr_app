...
import jaca.android.dev.BaseEventFetcher;
...
public class ServiceArtifact extends JaCaArtifact {
	 ...
	 * @param className service interface name
	 * @throws CartagoException
	 */
	protected void init(String action, String interfaceName) throws CartagoException {
		try {
			// get service interface class and service interface stub
			mStopped = false;
    		...
    		if (eventOp != null) {
	    		if (eventOp.getMethodName().equals(onServiceConnected)) {
					...
					try {
						...
						mServiceInstance = (IInterface) asInterface.invoke(null, service);
